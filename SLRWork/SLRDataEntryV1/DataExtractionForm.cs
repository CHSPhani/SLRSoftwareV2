...
namespace UoB.SLR.SLRDataEntryV1
{
    public partial class DataExtractionForm : Form
    {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txtQuery.Text.ToString()))
            {
                MessageBox.Show("Please enter query");
            }
            else
            {
                exModelClass = GetDataLayer.GetDataModelForQuery(txtQuery.Text.Trim().ToLower(), conn);
            }
            //Get Folder Path
            FolderBrowserDialog fbDialog = new FolderBrowserDialog();
            fbDialog.ShowDialog();
            ...
            List<string> existingfiles = Directory.GetFiles(folderPath, "querydata.xlsx", SearchOption.AllDirectories).ToList<string>();
            foreach (string fileDet in existingfiles)
            {
                System.IO.FileInfo fInfo = new FileInfo(fileDet);
                fInfo.Delete();
            ...
            try
            {
                ...
                //Add a new worksheet to workbook with the Datatable name
                Worksheet sheet = excelWorkBook.Sheets.Add();
                sheet.Name = "Query Result Details";
                ...
                sheet.SaveAs(folderPath + "\\querydata.xlsx");
            ...
            {
                MessageBox.Show(string.Format("There is a problem while creating querydata Excel sheet. The reason is {0}", ex.Message));
                Console.WriteLine("There is a problem in creating and saving querydata Excel sheet. the Reason is {0}", ex.Message);
                excelWorkBook.Close();
                excelApp.Quit();
            }
            MessageBox.Show("querydata details are exported");
        }
